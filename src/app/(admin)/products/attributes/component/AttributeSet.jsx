...
const AttributeSet = () => {
    ...
    const dynamicSKU = watch('dynamicsku')
    ...
    useEffect(() => {
        if (attributeSet) {
            ...
            setValue("dynamicsku", attributeSet.enableAutoSkuGeneration);
            handleModalShow()
        }
    }, [attributeSet])
    ...
    const handleCreateAttribute = handleSubmit(async (data) => {
        try {
            ...
            formData.append("EnableAutoSkuGeneration", data?.dynamicsku);
            formData.append("SkuPatternAttributeOrderMapping", Array.isArray(data.attributes) ? data.attributes?.map(attr => attr.value).join(', ') : '');
            ...
            <ConfirmationModal show={showModal} onHide={handleModalClose}>
                <form onSubmit={handleCreateAttribute}>
                    <ModalHeader closeButton>
                        <ModalTitle>{` ${attributeSetId ? "Edit" : "Add"} Attribute Set`}</ModalTitle>
                    </ModalHeader>
                            ...
                            <Controller
                                ...
                                render={({ field }) => {
                                    ...
                                    const handleRemoveTag = (removedTag) => {
                                        const updated = selectedTags.filter((tag) => tag.value !== removedTag);
                                    ...
                                    const handleDragEnd = (event) => {
                                        ...
                                        if (active.id !== over?.id) {
                                            const oldIndex = selectedTags.findIndex((tag) => tag.value === active.id);
                                            const newIndex = selectedTags.findIndex((tag) => tag.value === over?.id);
                                            const reordered = arrayMove(selectedTags, oldIndex, newIndex);
                                    ...
                                    return (
                                        <div>
                                            ...
                                            />
                                            {(dynamicSKU && selectedTags?.length > 0) ?
                                                <>

                                                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                                                        <SortableContext items={selectedTags.map((tag) => tag.value)} strategy={horizontalListSortingStrategy}>
                                                            <div className="d-flex flex-wrap gap-1 mt-2">
                                                                <SortableTags tag={{ label: "PARENT-SKU", value: "static" }} isStatic />

                                                                {selectedTags.map((tag) => (
                                                                    <SortableTags key={tag.value} tag={tag} onRemove={handleRemoveTag} />
                                                                ))}
                                                            </div>
                                                        </SortableContext>
                                                    </DndContext>
