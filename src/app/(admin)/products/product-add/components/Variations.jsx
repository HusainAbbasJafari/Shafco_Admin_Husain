...
const Variations = ({ register, setVariations, variations }) => {
    ...
    const columns = [
        ...
        {
            name: 'Variant Name',
            selector: (row) => `${row.attributes[0]?.value}/ ${row.attributes[1]?.value}`,
            // sortable: true,
        },
        {
            name: 'Price',
            ...
            cell: (row) => (
                <div className="mb-3">
                    ...
                    <input type="number" id="product-price" value={row.price} className="form-control" placeholder="Price" onChange={(e) => handleChangeById(row.tempId, 'price', e.target.value)} />
                </div>
            ),
            // sortable: true,
            width: '200px',
...

        {
            ...
            cell: (row) => (
                <div className="mb-3">
                    ...
                    <input type="number" id="product-quantity" className="form-control" placeholder="Quantity" onChange={(e) => handleChangeById(row.tempId, 'quantity', e.target.value)} />
                </div>
            ),
            // sortable: true,
            width: '200px',
...

        {
            ...
            cell: (row) => (
                <div className="mb-3">
                    ...
                    <input type="text" id="product-quantity" className="form-control" placeholder="SKU" onChange={(e) => handleChangeById(row.tempId, 'sku', e.target.value)}
                    />
                </div>
            ),
            // sortable: true,
            ...
            cell: (row) => (
                <div className="form-check form-switch">
                    <input
                        ...
                        role="switch"
                        id={`flexSwitch`}
                        onChange={(e) => handleChangeById(row.tempId, 'status', e.target.checked)}
                        checked={row.status}
                    />
                </div>
            ),
