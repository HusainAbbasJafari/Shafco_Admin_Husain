...
import { customSelectStyles, formatIDRCurrency, formatIDRCustom, mapToSelectOptions, unformatIDR } from '@/utils/other';
...
const ConfigurableProduct = () => {
    ...
    useEffect(() => {
        if (params?.encodeId) {
            const decodedId = atob(params.encodeId);
    ...
    const { data: product } = useProduct({ id: productId , variantId: null })
    ...
    useEffect(() => {
        const filteredAttributes = (Array.isArray(attributes)) && attributes?.map(attr => {
            const matchedAttribute = product?.productAttributes.find(
                variant => variant.attributesId === attr.attributeMasterId
            );

            const matchedValueIds = matchedAttribute?.values?.map(value => value.attributeValuesId);

            if (matchedAttribute) {
                return {
                    ...attr,
                    productAttributeValues: attr.productAttributeValues
                        .filter(value => matchedValueIds.includes(value.id))
                        .map((value) => ({
        ...
        if (Array.isArray(filteredAttributes) && filteredAttributes.length > 0) {
            filteredAttributes.forEach((item) => {
                setValue(item.attributeName, item.productAttributeValues || []);
            })
        }
    }, [attributes]);
    ...
    useEffect(() => {
        if (product) {
            const allVariations = Array.isArray(product.productVariants) && product.productVariants.map((item) => {
                return {
                    ...item,
                    price: formatIDRCustom(item?.price),
                    attributes: item.variantAttributesRes || []
                }
            })
            ...
            setValue("pagename", product?.metaPageName || "");
            setValue("productcategory", { label: product?.categoryName, value: product?.categoryId });
            setValue("brands", { label: product?.brandName, value: product?.brandId });
            setValue("attributeSet", { label: product?.attributeSetName, value: product?.attributeSetId })
            setValue("tags", product?.productTag || "");
            setVariations(allVariations || []);
            setQuillEditorContent(product.description || "");

            // needs handling
            setValue("sellingprice", formatIDRCustom(product?.sellingPrice.split(" ")[1]) || 0);
            setSellingPrice(formatIDRCustom(product?.sellingPrice.split(" ")[1]) || 0);
            setPrice(formatIDRCustom(product?.price?.split(" ")[1]) || 0);
            setValue("price", formatIDRCurrency(product?.price?.split(" ")[1]) || 0);
        }
    }, [product]);
    ...
    const handleImageUpload = async (data) => {
        try {
            ...
            variations?.forEach((item, index) => {
                ...
                const variantImage = item?.variantImagesWithId?.[0] || {};

                const productId = data?.productId || "";
                const productVariantId = variant?.productVariantId || "";
                const productImage = item?.image?.file || variantImage.imageUrl || '';
                const productImageName = item?.image?.file?.name || variantImage.imageName || '';
                const imageId = variantImage.imageId || '';

                formData.append(`ProductImages[${index}].ProductId`, productId);
                formData.append(`ProductImages[${index}].ProductVariantId`, productVariantId);
                formData.append(`ProductImages[${index}].ProductImage`, productImage);
                formData.append(`ProductImages[${index}].ProductImageName`, productImageName);
                formData.append(`ProductImages[${index}].imageId`, imageId);
            });

            formData.append("ProductId", product?.id)
            const res = await api.post("/api/Product/UpsertProductImages", formData);
        } catch (error) {
    ...
    const updateVariantStatus = async (id, status) => {
        try {
            ...
            const response = await api.post("/api/Product/UpdateProductVariantsStatus", formData);
        ...
        try {
            const variants = Array.isArray(variations) ? variations?.map(item => {
                return {
                    ...
                    price: unformatIDR(item.price),
                    sku: item.sku,
                    stock: item.stock,
                    isActive: item.isActive,
                    variantsId: item.varientsId?.startsWith('temp-') ? null : item.varientsId || null,
                };
            }) : null;
            ...
            if (response.status === 200) {
                ...
                if (Array.isArray(data)) {
                    const dataWithTempId = data.map((item, index) => ({
                        ...item,
                        varientsId: `temp-${index}`,
                        sku: item?.variantSKU,
                        price: formatIDRCurrency(item?.sellingPrice?.toString())
                    }));
                    // don't remove the temp from the varientsId if generated new this is required to keep check in backend
                                        ...
                                        render={({ field }) => (
                                            <Select
                                                ...
                                                onChange={(selectedOption) => {
                                                    ...
                                                    attributes?.forEach(attr => {
                                                        setValue(attr.attributeName, []); // Reset each attribute field
                                                    });
                                                }}
                                            />
                                        )}
